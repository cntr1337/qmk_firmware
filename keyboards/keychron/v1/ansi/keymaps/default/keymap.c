...
// 2. Definicja Twojego nowego przycisku
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    /* Warstwa MAC_BASE - tutaj podmienilem KC_F na F_MOUSE */
    [MAC_BASE] = LAYOUT_ansi_82(
        ...
        KC_CAPS,  KC_A,     KC_S,     KC_D,     F_MOUSE,  KC_G,     KC_H,     KC_J,     KC_K,     KC_L,     KC_SCLN,  KC_QUOT,              KC_ENT,             KC_HOME,
    ...
    /* Warstwa WIN_BASE - tutaj tez podmienilem KC_F na F_MOUSE */
...
// 3. Logika dzialania przycisku (dodana na koncu)
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        case F_MOUSE:
            if (record->event.pressed) {
                // Wcisniecie przycisku:
                tap_code(KC_F);             // Kliknij raz F
                register_code(KC_MS_BTN1);  // Wcisnij i trzymaj Lewy Przycisk Myszy
            } else {
                // Puszczenie przycisku:
                unregister_code(KC_MS_BTN1); // Pusc Lewy Przycisk Myszy
            }
            return false; // Zastepujemy standardowe dzialanie
            
        default:
            return true; // Inne klawisze dzialaja normalnie
    }
}